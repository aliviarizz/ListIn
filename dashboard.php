<?php
require_once 'includes/header.php'; // header.php sudah include db.php dan add_notification()
require_once 'includes/sidebar.php';
require_once 'includes/task_helper.php'; // Pastikan ini di-include
...
if (isset($conn) && $conn instanceof mysqli) {
    ...
    if ($stmt_stats_all) {
        ...
        if (!$stmt_stats_all->execute()) { error_log("Dashboard (Stats): Execute failed: (" . $stmt_stats_all->errno . ") " . $stmt_stats_all->error); }
        else {
            ...
            } else { error_log("Dashboard (Stats): get_result() failed: (" . $stmt_stats_all->errno . ") " . $stmt_stats_all->error); }
        }
        $stmt_stats_all->close();
    } else { error_log("Dashboard (Stats): Prepare failed: (" . $conn->errno . ") " . $conn->error); }
} else { error_log("Dashboard (Stats): Koneksi DB tidak valid atau tidak tersedia."); }
...
if (isset($_GET['filterPerformanceSubmit'])) {
    if (isset($_GET['filterPerformanceDateRange']) && !empty(trim($_GET['filterPerformanceDateRange']))) {
        ...
        if (count($range_perf) >= 1) {
            ...
            if (count($range_perf) == 2) {
                ...
                if (DateTime::createFromFormat('Y-m-d', $performance_start_date_val)) { // Pastikan start date valid sebelum diset ke end date
                    $performance_end_date_val = $performance_start_date_val;
                }
            }
        }
    }
    // Jika filter kustom disubmit, tidak ada preset yang aktif secara eksplisit
    // $active_preset_perf_val = ''; // Atau biarkan, JavaScript akan menghandle highlighting tombol
    ...
    switch ($active_preset_perf_val) {
        case 'today':
            $performance_start_date_val = $today_for_preset->format('Y-m-d');
            $performance_end_date_val = $today_for_preset->format('Y-m-d');
            break;
        case 'last7days':
            $performance_end_date_val = $today_for_preset->format('Y-m-d');
            $performance_start_date_val = $today_for_preset->modify('-6 days')->format('Y-m-d');
            break;
        case 'this_month':
            $performance_start_date_val = $today_for_preset->format('Y-m-01');
            $performance_end_date_val = $today_for_preset->format('Y-m-t');
            break;
    }
}

$line_chart_labels_php = [];
$line_chart_data_completed_php = [];
if (isset($conn) && $conn instanceof mysqli) {
    error_log("Dashboard (Performa): Memulai pengambilan data chart. Rentang: $performance_start_date_val s/d $performance_end_date_val");
    try {
        ...
        if ($current_date_loop_obj && $end_date_loop_obj) {
            ...
            while ($current_date_loop_obj <= $end_date_loop_obj) {
                ...
                if($stmt_completed_on_date) {
                    ...
                    if(!$stmt_completed_on_date->execute()){ error_log("Dashboard (Performa): Execute failed: " . $stmt_completed_on_date->error . " for date " . $date_str_loop); }
                    else {
                        ...
                        } else if ($result_completed_on_date === false) { error_log("Dashboard (Performa): get_result() failed: " . $stmt_completed_on_date->error . " for date " . $date_str_loop); }
                    }
                    $stmt_completed_on_date->close();
                } else { error_log("Dashboard (Performa): Prepare failed: " . $conn->error . " untuk query tanggal " . $date_str_loop); }
                $line_chart_data_completed_php[] = $tasks_completed_on_day;
                
                $current_date_loop_obj->add($interval_one_day); $loop_count++;
                if ($loop_count > 90 ) { // Batasi loop misalnya 90 hari
                    if ($current_date_loop_obj <= $end_date_loop_obj) { // Jika masih ada data setelah batas
                        $line_chart_labels_php[] = "..."; $line_chart_data_completed_php[] = null;
                    }
                    break; 
                }
            }
        } else { error_log("Dashboard (Performa): Invalid date format. Start: '$performance_start_date_val', End: '$performance_end_date_val'"); }
    } catch (Exception $e) { error_log("Dashboard (Performa): Exception: " . $e->getMessage()); }
} else { error_log("Dashboard (Performa): Koneksi DB tidak valid."); }

// Pastikan ada label jika data kosong agar chart tidak error
...
$performance_chart_data_php_final = [
    ...
    'datasets' => [[
        'label' => 'Tugas Selesai',
        'data' => $line_chart_data_completed_php,
        'borderColor' => '#7e47b8',
        'backgroundColor' => 'rgba(126, 71, 184, 0.1)',
        'fill' => true,
        'tension' => 0.2
    ]]
];

// Ambil SEMUA tugas aktif untuk daftar di kolom kanan
function get_dashboard_all_active_tasks($db_connection, $current_user_id_func) {
    ...
    if (!$db_connection || !($db_connection instanceof mysqli)) {
        error_log("Fungsi get_all_active_tasks: Koneksi DB tidak valid.");
    ...
    if (!$stmt) {
        error_log("Fungsi get_all_active_tasks: Prepare failed: " . $db_connection->error);
        return $active_tasks_data;
    }
    $stmt->bind_param("i", $current_user_id_func);
    if (!$stmt->execute()) {
        error_log("Fungsi get_all_active_tasks: Execute failed: " . $stmt->error);
        $stmt->close(); return $active_tasks_data;
    }
    $result = $stmt->get_result();
    if ($result) {
        while ($row = $result->fetch_assoc()) { $active_tasks_data[] = $row; }
    } else { error_log("Fungsi get_all_active_tasks: get_result() failed: " . $stmt->error); }
    $stmt->close();
    return $active_tasks_data;
}
...
<main class="main">
    <div class="dashboard-grid">
        <div class="page-title-container">
            <h2 class="page-title" style="margin-bottom:15px;">Dasbor Saya</h2> 
        </div>
                    ...
                    <?php if ($total_tasks_all_time > 0): ?>
                        ...
                        <div class="progress-item">
                            <div class="progress-circle"
                                 ...
                                 <!-- Teks persentase akan di-generate oleh ::before dari CSS -->
                            </div>
        ...
        <!-- Kolom Kanan (Hanya Tugas Aktif, bisa scroll) -->
        <div class="dashboard-right-column">
            <section class="widget" style="height:100%;"> <!-- Widget ini akan mengisi tinggi kolom kanan -->
                <h3>Tugas Aktif (Total: <?php echo count($dashboard_all_active_tasks); ?>)</h3>
                <div class="widget-content-area" id="dashboard-active-tasks-list" style="padding-top:0;"> <!-- Area ini yang scroll -->
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (lineCtx && typeof Chart !== 'undefined') {
        ...
        if (performanceData && performanceData.labels && Array.isArray(performanceData.labels) &&
            ...
            // Cek apakah ada label valid (bukan 'Error' atau 'Tidak Ada Data') DAN ada data numerik yang valid
        ...
        if (hasValidLineData) {
            ...
            // console.log("Data line chart tidak valid atau kosong, chart tidak dirender.");
        }
    }

    // JavaScript untuk tombol preset performa (sudah ada di respons sebelumnya)
    ...
    perfPresetButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            // Form akan disubmit oleh tombol karena typenya submit
            if (perfDateRangeInput) perfDateRangeInput.value = ''; 
            
            // Hapus kelas active dari semua tombol preset di grup ini
